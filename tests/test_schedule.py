...
def test_far_tier_scans_only_the_base_weekend():
    # 182 days out and divisible by 7, so the weekly tier is due today.
    today = FRIDAY - timedelta(days=182)
    queries = [query for query in schedule.due_today(spec(), today) if query.depart_date == FRIDAY]
...
def test_a_weekend_comes_up_on_its_tier_interval():
    ...
    assert due == list(range(33, 91, 3))  # every third day, never two days running
